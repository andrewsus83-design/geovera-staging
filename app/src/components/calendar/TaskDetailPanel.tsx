    ...
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        {[
          ...
          <div key={s.label} className="rounded-lg bg-gray-50 dark:bg-gray-800/60 p-2 text-center">
          ...
          >
            ...
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2 min-w-0">
                <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full ${platformColor[r.platform] ?? "bg-gray-100 text-gray-600"}`}>
                  {r.platformIcon} {r.platform}
                </span>
                <span className="text-[10px] font-medium text-gray-700 dark:text-gray-300 truncate">@{r.author}</span>
              </div>
              <div className="flex items-center gap-1 flex-shrink-0">
                <span className={`text-[9px] font-bold ${scoreColor(r.authorScore)}`}>#{r.authorScore}</span>
                {r.status === "approved" && (
            ...
            {/* Original comment */}
            <p className="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed mb-2 italic">
            ...
            {editingId === r.id ? (
              <div className="space-y-1.5">
                <textarea
                  ...
                  rows={3}
                  className="w-full rounded-lg border border-brand-300 bg-white px-3 py-2 text-xs text-gray-700 outline-none focus:border-brand-500 dark:border-brand-600 dark:bg-gray-800 dark:text-gray-300 resize-none"
                />
                <div className="flex gap-1.5">
                  <button
                    onClick={() => saveEdit(r.id)}
                    className="flex-1 rounded-lg bg-brand-500 py-1.5 text-[11px] font-medium text-white hover:bg-brand-600"
                  >
                    Save & Approve
                  </button>
                  <button
                    onClick={() => setEditingId(null)}
                    className="rounded-lg border border-gray-200 px-3 py-1.5 text-[11px] text-gray-500 hover:bg-gray-50 dark:border-gray-700"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <div className="rounded-lg bg-gray-50 dark:bg-gray-800/60 px-3 py-2 mb-2">
                <div className="flex items-center gap-2 mb-1.5">
                  <p className="text-[10px] font-medium text-gray-400">AI Draft Reply</p>
                  <span className="inline-flex items-center gap-1 rounded-full bg-purple-50 px-1.5 py-0.5 text-[9px] font-medium text-purple-600 dark:bg-purple-500/10 dark:text-purple-400">
                ...
                <p className="text-[11px] text-gray-700 dark:text-gray-300 leading-relaxed">{r.draftReply}</p>
              </div>
            )}
              ...
              <div className="flex gap-1.5">
                {r.status !== "approved" && (
                  <button
                    ...
                    className="flex-1 rounded-lg bg-green-500 py-1.5 text-[11px] font-medium text-white hover:bg-green-600 transition-colors"
                  >
                    ✓ Approve
                  </button>
                )}
                <button
                  onClick={() => startEdit(r)}
                  className="flex-1 rounded-lg border border-brand-200 py-1.5 text-[11px] font-medium text-brand-600 hover:bg-brand-50 transition-colors dark:border-brand-500/30 dark:text-brand-400"
                >
                  Edit
                </button>
                {r.status !== "approved" && (
                  <button
                    ...
                    className="rounded-lg border border-purple-200 px-2.5 py-1.5 text-[11px] text-purple-600 hover:bg-purple-50 transition-colors dark:border-purple-500/30 dark:text-purple-400"
                  >
                    AI Feedback
                  </button>
                )}
            ...
            {/* AI Feedback box */}
            {feedbackId === r.id && (
              <div className="mt-1.5 rounded-lg border border-purple-200 bg-purple-50/60 dark:border-purple-500/20 dark:bg-purple-500/5 p-2.5">
                <p className="text-[10px] font-medium text-purple-700 dark:text-purple-300 mb-1.5">
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full rounded-lg border border-purple-200 bg-white px-2.5 py-1.5 text-xs text-gray-700 outline-none focus:border-purple-400 dark:border-purple-500/30 dark:bg-gray-800 dark:text-gray-300 resize-none"
                />
                <div className="flex gap-1.5 mt-1.5">
                  <button
                    onClick={() => submitFeedback(r.id)}
                    disabled={!feedbackText.trim()}
                    className="flex-1 rounded-lg bg-purple-500 py-1.5 text-[11px] font-medium text-white hover:bg-purple-600 disabled:opacity-40 transition-colors"
                  >
                    Regenerate
                  </button>
                  <button
                    onClick={() => { setFeedbackId(null); setFeedbackText(""); }}
                    className="rounded-lg border border-purple-200 px-2.5 py-1.5 text-[11px] text-purple-500 hover:bg-purple-50 transition-colors dark:border-purple-500/30"
                  >
                    Cancel
                  </button>
                </div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div>
          <h4 className="text-xs font-medium uppercase text-gray-400 mb-1.5">What to do</h4>
          <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{task.description}</p>
        </div>

        {task.content?.caption && (
          <div>
            <h4 className="text-xs font-medium uppercase text-gray-400 mb-1.5">Caption</h4>
        ...
        {task.content?.hashtags && task.content.hashtags.length > 0 && (
          <div>
            <h4 className="text-xs font-medium uppercase text-gray-400 mb-1.5">Hashtags</h4>
            <div className="flex flex-wrap gap-1.5">
              {task.content.hashtags.map((tag, i) => (
                <span key={i} className="inline-flex items-center rounded-full bg-brand-50 px-2 py-0.5 text-xs text-brand-700 dark:bg-brand-500/10 dark:text-brand-400">
                  {tag}
                </span>
              ))}
            </div>
        ...
        {showScheduler && (
          <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800">
            <h4 className="text-xs font-medium text-gray-600 dark:text-gray-300 mb-2">Schedule Publish</h4>
