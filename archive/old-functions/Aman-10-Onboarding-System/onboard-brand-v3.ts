...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ============================================================
    if (step === 1) {
      const { brand_name, category, business_type, country, tier, billing_cycle } = body;
      ...
      // Create brand with tier and billing info
      ...
      return new Response(JSON.stringify({
        ...
        pricing: pricing,
        next_step: 2,
        message: "Brand created! Please complete step 2 for social media links.",
      }), {
        status: 201,
    ...
    if (step === 2) {
      if (!body.brand_id) {
        errors.push("brand_id wajib untuk Step 2");
      }
      ...
      return new Response(JSON.stringify({
        ...
        brand: updatedBrand,
        next_step: 3,
        message: "Social media links updated! Please confirm your brand setup.",
      }), {
        status: 200,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    ...
    // ============================================================
    // STEP 3: CONFIRMATION (30-day brand lock warning)
    // ============================================================
    if (step === 3) {
      if (!body.brand_id) {
      ...
      const { error: confirmError } = await supabase
        ...
        .insert({
          ...
          confirmed_tier: brand.subscription_tier,
          confirmed_billing_cycle: brand.billing_cycle,
      ...
      return new Response(JSON.stringify({
        ...
        locked_until: brand.brand_locked_until,
