...
![Insert hero image here with markdown: ![${brandName} premium product showcase](Professional commercial photography of ${brandName} products, ${geminiData.category}, brand colors [from research], premium setting, natural lighting, photorealistic 4K)]
...
"[Scene description], ${brandName} brand identity with [LOGO DESCRIPTION from Perplexity], brand colors: [EXACT HEX CODES from Perplexity], [DESIGN AESTHETIC from Perplexity], [PHOTOGRAPHY STYLE from Perplexity], [VISUAL MOOD from Perplexity], ${country || 'Indonesia'} cultural elements, ${geminiData.category} product category, [LIGHTING STYLE from Perplexity], photorealistic 4K quality, matches brand's social media visual style"
...
![${brandName} product showcase with brand design](${brandName} ${geminiData.category} product showcase, brand colors from Perplexity: [EXACT HEX CODES], [DESIGN STYLE from Perplexity] aesthetic, ${country || 'Indonesia'} cultural elements, [LOGO ELEMENTS from Perplexity] visible, [PHOTOGRAPHY STYLE from Perplexity], photorealistic 4K quality, brand identity consistent with Perplexity research)
...
1. **Hero Product Shot**
![${brandName} hero product showcase](Professional commercial photography of ${brandName} flagship product, ${geminiData.category}, shot on premium surface with brand colors [exact hex from research], natural lighting, 45-degree angle, photorealistic 4K, brand packaging prominently displayed, [cultural elements from research], shallow depth of field, premium aesthetic)
...
// Step 5: Generate real images via DALL-E and upload to Supabase Storage
async function step5_generate_images(reportMarkdown: string, slug: string): Promise<string> {
  const OPENAI_API_KEY = Deno.env.get('OPENAI_API_KEY')!;
  const SUPABASE_URL = Deno.env.get('SUPABASE_URL')!;
  const SUPABASE_SERVICE_KEY = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!;

  // Extract all ![alt](prompt) patterns
  const imagePattern = /!\[([^\]]*)\]\(([^)]+)\)/g;
  const matches: { full: string; alt: string; prompt: string }[] = [];
  let match;
  while ((match = imagePattern.exec(reportMarkdown)) !== null) {
    // Skip if it's already a URL (http/https)
    if (match[2].startsWith('http')) continue;
    matches.push({ full: match[0], alt: match[1], prompt: match[2] });
  }

  if (matches.length === 0) {
    console.log('No DALL-E prompts found in report');
    return reportMarkdown;
  }

  // Limit to max 6 images to control cost and time
  const imagesToGenerate = matches.slice(0, 6);
  console.log(`Generating ${imagesToGenerate.length} images via DALL-E 3...`);

  let updatedMarkdown = reportMarkdown;

  // Process images in parallel (batches of 3 to avoid rate limits)
  for (let i = 0; i < imagesToGenerate.length; i += 3) {
    const batch = imagesToGenerate.slice(i, i + 3);
    const results = await Promise.allSettled(
      batch.map(async (img, batchIdx) => {
        const imgIdx = i + batchIdx;
        try {
          // Truncate prompt to 4000 chars (DALL-E limit)
          const truncatedPrompt = img.prompt.substring(0, 3900);
          ...
          if (!dalleData.data?.[0]?.url) {
            console.error(`DALL-E error for image ${imgIdx}:`, JSON.stringify(dalleData.error || dalleData));
            return null;
          }
          ...
          const storagePath = `${slug}/image-${imgIdx}.png`;
          ...
          if (!uploadResponse.ok) {
            ...
            console.error(`Storage upload error for image ${imgIdx}:`, err);
            return null;
          }

          // Return the public URL
          const publicUrl = `${SUPABASE_URL}/storage/v1/object/public/report-images/${storagePath}`;
          console.log(`Image ${imgIdx} uploaded: ${publicUrl}`);
          return { original: img.full, alt: img.alt, url: publicUrl };
        } catch (err) {
          console.error(`Image ${imgIdx} generation failed:`, err);
          return null;
        }
      })
    );

    // Replace prompts with actual URLs
    for (const result of results) {
      if (result.status === 'fulfilled' && result.value) {
        const { original, alt, url } = result.value;
        updatedMarkdown = updatedMarkdown.replace(original, `![${alt}](${url})`);
      }
    }
  }
...
async function uploadReportToStorage(slug: string, htmlContent: string): Promise<string> {
  ...
  if (!uploadResponse.ok) {
    ...
    throw new Error(`Failed to upload report to storage: ${err}`);
  }

  return `${SUPABASE_URL}/storage/v1/object/public/reports/${storagePath}`;
}
...
function generateStaticHTML(data: StaticReportData): string {
  ...
  let htmlContent = data.report_markdown
    .replace(/!\[(.*?)\]\((.*?)\)/gim, '<figure class="img-card"><img src="$2" alt="$1" loading="lazy" onerror="this.parentElement.style.display=\'none\'"><figcaption>$1</figcaption></figure>')
    .replace(/^### (.*$)/gim, '<h3>$1</h3>')
    .replace(/^## (.*$)/gim, '<h2><span class="h2-icon"><svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg></span>$1</h2>')
    .replace(/^# (.*$)/gim, '<h1>$1</h1>')
    ...
    .replace(/\n\n/g, '</p><p>')
    .replace(/\n/g, '<br>')
    .replace(/<br>---<br>/g, '<div class="section-divider"></div>');
...
<meta property="og:title" content="${data.brand_name} Intelligence Report">
...
:root{
  --brand-25:#f0fdf4;--brand-50:#dcfce7;--brand-100:#bbf7d0;--brand-200:#86efac;--brand-300:#4ade80;
  --brand-400:#22c55e;--brand-500:#16a34a;--brand-600:#15803d;--brand-700:#166534;--brand-800:#14532d;
  --gray-25:#fcfcfd;--gray-50:#f9fafb;--gray-100:#f2f4f7;--gray-200:#e4e7ec;--gray-300:#d0d5dd;
  --gray-400:#98a2b3;--gray-500:#667085;--gray-600:#475467;--gray-700:#344054;--gray-800:#1d2939;--gray-900:#101828;
  --success-50:#ecfdf3;--success-500:#12b76a;--success-700:#027a48;
  --warning-50:#fffaeb;--warning-500:#f79009;--warning-700:#b54708;
  --error-50:#fef3f2;--error-500:#f04438;--error-700:#b42318;
  ...
  --bp:${brandColors.primary};--bs:${brandColors.secondary};--ba:${brandColors.accent}
}
*{margin:0;padding:0;box-sizing:border-box}
::selection{background:var(--brand-100);color:var(--brand-800)}
body{font-family:'Inter',-apple-system,BlinkMacSystemFont,system-ui,sans-serif;color:var(--gray-700);background:var(--gray-50);line-height:1.6;-webkit-font-smoothing:antialiased;font-size:15px}

/* === Top Bar === */
.topbar{background:#fff;border-bottom:1px solid var(--gray-200);padding:12px 24px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:50;backdrop-filter:blur(8px);background:rgba(255,255,255,0.95)}
.topbar-left{display:flex;align-items:center;gap:12px}
.topbar-logo{width:32px;height:32px;background:var(--brand-500);border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:14px}
.topbar-brand{font-weight:600;font-size:15px;color:var(--gray-900)}
.topbar-sep{color:var(--gray-300);font-weight:300}
.topbar-page{font-weight:500;font-size:14px;color:var(--gray-500)}
.topbar-right{display:flex;align-items:center;gap:8px}
.topbar-btn{display:inline-flex;align-items:center;gap:6px;padding:8px 14px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.15s;border:1px solid var(--gray-200);background:#fff;color:var(--gray-700)}
.topbar-btn:hover{background:var(--gray-50);border-color:var(--gray-300);transform:translateY(-1px);box-shadow:var(--shadow-sm)}
.topbar-btn.primary{background:var(--brand-500);color:#fff;border-color:var(--brand-500)}
.topbar-btn.primary:hover{background:var(--brand-600);border-color:var(--brand-600);box-shadow:0 4px 12px rgba(22,163,74,0.25)}
.topbar-btn svg{width:16px;height:16px}

/* === Hero Card === */
.hero-card{max-width:860px;margin:32px auto 0;padding:0 24px}
.hero-inner{background:#fff;border:1px solid var(--gray-200);border-radius:12px;padding:40px 48px;box-shadow:var(--shadow-sm);position:relative;overflow:hidden}
.hero-inner::before{content:'';position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(90deg,var(--brand-500),var(--brand-300),var(--brand-500))}
.hero-badge{display:inline-flex;align-items:center;gap:6px;background:var(--brand-25);border:1px solid var(--brand-200);color:var(--brand-700);padding:4px 12px;border-radius:9999px;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.04em;margin-bottom:20px}
.hero-badge .dot{width:6px;height:6px;border-radius:50%;background:var(--brand-500);animation:pulse 2s infinite}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.3}}
.hero-inner h1{font-size:clamp(1.75rem,4vw,2.5rem);font-weight:800;color:var(--gray-900);letter-spacing:-0.025em;line-height:1.15;margin-bottom:8px}
.hero-sub{font-size:15px;color:var(--gray-500);font-weight:400;margin-bottom:24px}
.meta-grid{display:flex;gap:12px;flex-wrap:wrap}
.meta-item{display:flex;align-items:center;gap:8px;background:var(--gray-50);border:1px solid var(--gray-200);padding:8px 16px;border-radius:8px;font-size:13px}
.meta-item .meta-label{color:var(--gray-400);font-weight:500}
.meta-item .meta-value{color:var(--gray-900);font-weight:600}
...
.content{}
.content h1{font-size:1.5rem;font-weight:800;color:var(--gray-900);margin:32px 0 16px;letter-spacing:-0.02em;line-height:1.3}
.content h2{display:flex;align-items:center;gap:10px;font-size:1.1rem;font-weight:700;color:var(--gray-900);margin:32px 0 0;padding:20px 24px;background:#fff;border:1px solid var(--gray-200);border-bottom:none;border-radius:12px 12px 0 0;box-shadow:var(--shadow-xs);letter-spacing:-0.01em;line-height:1.4}
.content h2 .h2-icon{display:flex;align-items:center;justify-content:center;width:32px;height:32px;background:var(--brand-50);border-radius:8px;color:var(--brand-500);flex-shrink:0}
.content h2+p,.content h2+ul,.content h2+ol,.content h2+figure,.content h2+.section-divider{border:1px solid var(--gray-200);border-top:none}
.content p{color:var(--gray-600);margin-bottom:0;padding:12px 24px;font-size:14px;line-height:1.75;background:#fff}
.content p+p{padding-top:0}
.content p:last-child,.content p+h2,.content p+h1{margin-bottom:0}

/* Auto-close section cards */
.content h2~p:last-of-type,.content h2~ul:last-of-type,.content h2~ol:last-of-type{border-radius:0 0 12px 12px;padding-bottom:20px;margin-bottom:24px;box-shadow:var(--shadow-xs)}

.content strong{color:var(--gray-900);font-weight:600}
.content em{color:var(--gray-500);font-style:italic}
.content ul,.content ol{margin:0;padding:12px 24px 12px 48px;color:var(--gray-600);background:#fff;font-size:14px}
.content li{margin-bottom:6px;line-height:1.7;padding-left:4px}
.content li::marker{color:var(--brand-400)}
.content li.arrow{list-style:none;padding-left:24px;position:relative;margin-left:-24px}
.content li.arrow::before{content:'\\2192';position:absolute;left:0;color:var(--brand-500);font-weight:600}
.content li.todo{list-style:none;padding:10px 14px;background:var(--gray-50);border:1px solid var(--gray-200);border-radius:8px;margin:6px 0;font-weight:500;color:var(--gray-900);margin-left:-24px;display:flex;align-items:center;gap:8px}
.content li.todo::before{content:'';width:16px;height:16px;border:2px solid var(--brand-400);border-radius:4px;flex-shrink:0}

/* === Tables === */
.content table{width:100%;border-collapse:collapse;margin:16px 0 24px;font-size:13px;border:1px solid var(--gray-200);border-radius:12px;overflow:hidden;background:#fff;box-shadow:var(--shadow-xs)}
.content thead{background:var(--gray-50)}
.content th{padding:12px 16px;text-align:left;font-weight:600;color:var(--gray-700);font-size:12px;text-transform:uppercase;letter-spacing:0.05em;border-bottom:1px solid var(--gray-200)}
.content td{padding:12px 16px;border-bottom:1px solid var(--gray-100);color:var(--gray-600)}
.content tbody tr:last-child td{border-bottom:none}
.content tbody tr:hover{background:var(--gray-25)}

/* === Section Divider === */
.section-divider{height:0;margin:24px 0;border:none}

/* === Images === */
figure.img-card{margin:16px 0 24px;border-radius:12px;overflow:hidden;border:1px solid var(--gray-200);background:#fff;box-shadow:var(--shadow-sm)}
figure.img-card img{width:100%;height:auto;display:block;object-fit:cover;max-height:420px}
figure.img-card figcaption{padding:10px 16px;font-size:12px;color:var(--gray-500);border-top:1px solid var(--gray-100);background:var(--gray-25);font-weight:500}

/* === CTA Section === */
.cta-card{background:#fff;border:1px solid var(--gray-200);border-radius:12px;padding:40px 32px;text-align:center;margin:32px 0;box-shadow:var(--shadow-sm);position:relative;overflow:hidden}
.cta-card::before{content:'';position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(90deg,var(--brand-400),var(--brand-500),var(--brand-600))}
.cta-card h3{font-size:1.25rem;color:var(--gray-900);margin-bottom:8px;font-weight:700}
.cta-card p{color:var(--gray-500);margin-bottom:20px;font-size:14px;padding:0}
.cta-btn{display:inline-flex;align-items:center;gap:8px;background:var(--brand-500);color:#fff;padding:10px 24px;border-radius:8px;text-decoration:none;font-weight:600;font-size:14px;transition:all 0.15s;border:none;cursor:pointer}
.cta-btn:hover{background:var(--brand-600);transform:translateY(-1px);box-shadow:0 4px 12px rgba(22,163,74,0.25)}

/* === Footer === */
footer{max-width:860px;margin:0 auto;padding:0 24px 48px}
.footer-inner{border-top:1px solid var(--gray-200);padding:24px 0;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px}
.footer-left{display:flex;align-items:center;gap:10px}
.footer-logo{width:28px;height:28px;background:var(--brand-500);border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-weight:700;font-size:12px}
.footer-info{font-size:13px;color:var(--gray-500)}
.footer-info strong{color:var(--gray-700);font-weight:600}
.footer-right{display:flex;align-items:center;gap:16px}
.footer-link{font-size:13px;color:var(--brand-600);text-decoration:none;font-weight:500;transition:color 0.15s}
.footer-link:hover{color:var(--brand-700)}
.footer-powered{text-align:center;width:100%;font-size:11px;color:var(--gray-400);margin-top:8px}

/* === Print === */
@media print{
  .topbar,.fab-actions{display:none}
  body{background:#fff}
  .hero-card{margin-top:0;padding:0}
  .hero-inner{box-shadow:none;border:none;border-radius:0;padding:24px 0}
  .hero-inner::before{display:none}
  .main{padding:0}
  .content h2{box-shadow:none;border:none;padding:12px 0;border-bottom:2px solid var(--gray-200);border-radius:0}
  .content p,.content ul,.content ol{border:none;padding-left:0;padding-right:0;background:transparent}
  .content h2~p:last-of-type,.content h2~ul:last-of-type,.content h2~ol:last-of-type{box-shadow:none;border-radius:0}
  figure.img-card{box-shadow:none}
  footer{padding:0}
  .footer-inner{border:none;padding:16px 0}
}
...
@media(max-width:640px){
  ...
  .hero-card{padding:0 16px;margin-top:20px}
  ...
  .content h2{padding:16px 16px;font-size:1rem}
  .content p{padding:10px 16px;font-size:13px}
  .content ul,.content ol{padding:10px 16px 10px 40px}
  .fab-actions{bottom:16px;right:16px;gap:6px}
  .fab-actions button{padding:8px 12px;font-size:12px}
...
<span class="topbar-brand">GeoVera</span>
<span class="topbar-sep">/</span>
<span class="topbar-page">Intelligence Report</span>
</div>
...
<button class="topbar-btn primary" onclick="navigator.share?.({title:'${data.brand_name} Report',url:location.href}).catch(()=>{navigator.clipboard.writeText(location.href);this.textContent='Copied!'})">
...
<div class="hero-badge"><span class="dot"></span>AI-Generated Intelligence Report</div>
<h1>${data.brand_name}</h1>
<p class="hero-sub">Brand Intelligence Report &middot; ${formattedDate}</p>
<div class="meta-grid">
<div class="meta-item">
<span class="meta-label">Company</span>
<span class="meta-value">${data.parent_company}</span>
</div>
<div class="meta-item">
...
<!-- CTA -->
<div class="cta-card">
<h3>Want your own brand intelligence report?</h3>
<p>Get AI-powered insights for your brand in minutes.</p>
<a href="https://geovera.xyz/onboarding" class="cta-btn">
Get Started
...
<div class="footer-info"><strong>GeoVera</strong> &middot; &copy; 2026</div>
</div>
<div class="footer-right">
<a href="https://geovera.xyz" class="footer-link">geovera.xyz</a>
</div>
<div class="footer-powered">Powered by Perplexity AI &middot; Gemini &middot; Claude &middot; GPT-4o &middot; DALL-E 3</div>
</div>
</footer>
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Step 5: Generate real images via DALL-E
    console.log('🎨 Step 5: Generating images via DALL-E 3...');
    let reportWithImages: string;
    try {
      reportWithImages = await step5_generate_images(finalReport, slug);
      console.log('✅ Step 5 Complete - Images generated\n');
    } catch (imgError) {
      console.error('⚠️ Image generation failed, using report without images:', imgError);
      reportWithImages = finalReport;
    }
    ...
    const staticHTML = generateStaticHTML({
      ...
      report_markdown: reportWithImages
    });
    ...
    return new Response(
      JSON.stringify({
        ...
        metadata: {
          ...
          report_length: reportWithImages.length,
